...
from (select emp_name, salary, dept_id
	  from emp
      where salary = (select max(salary) from emp)) a left join dept d on a.dept_id = d.dept_id;
...
select 	*
from	emp
where	salary < (select avg(salary) from emp where job_id = 'ST_CLERK') and job_id != 'ST_CLERK'
order by salary desc;
-- 2817.647059
...
/* *************************************************************************************************
상관(연관) 쿼리
- 메인쿼리문의 테입블의 값을 서브쿼리의 조건에서 사용하는 쿼리.
...
************************************************************************************* */
​
-- 직원이 한명이상 있는 부서의 부서ID(dept.dept_id)와 
--    이름(dept.dept_name),위치(dept.loc)를 조회
--  직원(자식)이 부서(부모)을 참조. 부서(부모)의 값이 직원(자식)에서 참조되고 있냐?
​
select * from dept
...
​
-- 부서(dept)에서 연봉(emp.salary)이 13000이상인 한명이라도 있는 
-- 부서의 부서ID(dept.dept_id)와 이름(dept.dept_name), 위치(dept.loc)를 조회
...
​
select * from orders where cust_id = 110;
​
